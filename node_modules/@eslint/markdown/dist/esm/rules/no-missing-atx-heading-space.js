            ...
            paragraph(node) {
                if (node.children && node.children.length > 0) {
                    ...
                    lines.forEach((line, idx) => {
                        const lineNum = firstTextChild.position.start.line + idx;
                        const match = headingPattern.exec(line);
                        if (!match) {
                            return;
                        }
                        const hashes = match[1];
                        const startColumn = firstTextChild.position.start.column;
                        context.report({
                            loc: {
                            ...
                            fix(fixer) {
                                const offset = firstTextChild.position.start.offset +
