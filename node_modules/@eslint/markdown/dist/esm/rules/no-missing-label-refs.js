...
function findMissingReferences(node, nodeText) {
    ...
    while ((match = labelPattern.exec(nodeText))) {
        ...
        const { lineOffset: startLineOffset, columnOffset: startColumnOffset } = findOffsets(nodeText, labelIndices[0]);
        const { lineOffset: endLineOffset, columnOffset: endColumnOffset } = findOffsets(nodeText, labelIndices[1]);
        missing.push({
            label: label.trim(),
            position: {
                start: {
                    line: nodeStartLine + startLineOffset,
                    column: startLineOffset > 0
                        ? startColumnOffset + 1
                        : nodeStartColumn + startColumnOffset,
                },
                end: {
                    line: nodeStartLine + endLineOffset,
                    column: endLineOffset > 0
                        ? endColumnOffset + 1
...
export default {
    ...
    create(context) {
        ...
        return {
            ...
            text(node) {
                allMissingReferences.push(...findMissingReferences(node, sourceCode.getText(node)));
            },
            definition(node) {
                /*
