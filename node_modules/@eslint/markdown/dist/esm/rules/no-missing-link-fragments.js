...
export default {
    ...
    create(context) {
        const { allowPattern: allowPatternString, ignoreCase } = context.options[0];
        ...
        return {
            heading(node) {
                ...
                let baseId;
                const customIdMatch = rawHeadingText.match(customHeadingIdPattern);
                if (customIdMatch) {
            ...
            html(node) {
                ...
                const textWithoutComments = htmlText.replace(htmlCommentPattern, "");
                // Then look for IDs in the remaining text
                for (const match of textWithoutComments.matchAll(htmlIdNamePattern)) {
