...
function isInCodeSpan(matchIndex, codeSpans) {
    return codeSpans.some(span => matchIndex >= span.startOffset && matchIndex < span.endOffset);
}
/**
        ...
        return {
            paragraph(node) {
                ...
                while ((match = reversedPattern.exec(text)) !== null) {
                    ...
                    const { lineOffset: startLineOffset, columnOffset: startColumnOffset, } = findOffsets(text, matchIndex);
                    const { lineOffset: endLineOffset, columnOffset: endColumnOffset, } = findOffsets(text, matchIndex + matchLength);
                    const baseColumn = 1;
                    const nodeStartLine = node.position.start.line;
                    const nodeStartColumn = node.position.start.column;
                    const startLine = nodeStartLine + startLineOffset;
                    const endLine = nodeStartLine + endLineOffset;
                    const startColumn = (startLine === nodeStartLine
                        ? nodeStartColumn
                        : baseColumn) + startColumnOffset;
                    const endColumn = (endLine === nodeStartLine
                        ? nodeStartColumn
                    ...
                    context.report({
                        ...
                        fix(fixer) {
                            const startOffset = node.position.start.offset + matchIndex;
